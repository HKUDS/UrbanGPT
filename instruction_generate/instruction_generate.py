...
args.add_argument('-dataset_name', default='NYCmulti', type=str)
...
for i in range(train_len):
    ...
    for region_index in range(0, data.shape[2], 1):
        ...
        if type == 2 or type == 3 or type == 4:
            if args.for_supervised or args.for_test==False:
                region_index_new = selected_regions[0:80][region_index+args.region_start]
            else:
                region_index_new = selected_regions[80:160][region_index + args.region_start]
        elif type == 1:
            if args.for_supervised or args.for_test==False:
                region_index_new = region_index + args.region_start
            else:
                region_index_new = region_index + args.region_start + 80
        else:
            region_index_new = region_index + args.region_start
        ...
        if type == 1:
            value_of_human = "Given the historical data for taxi flow over 12 time steps in a specific region of New York City, " \
                             ...
                             ". Here is the region information:" + region_decode_ori(region_index_new, type, zone_json) + "Now we want to predict " \
            ...
            value_of_human = "Given the historical data for bike flow over 12 time steps in a specific region of New York City, " \
                             ...
                             ". The recording time of the historical data is " + time_decode(data_gpt, args, type) + \
                             ". Here is the region information:" + region_decode_ori(region_index_new, type, region_json) + "We now aim to predict " \
            ...
            value_of_human = "Given the historical data for crime over 12 time steps in a specific region of New York City, " \
                             ...
                             ". The recording time of the historical data is " + time_decode(data_gpt, args, type) + \
                             ". Here is the region information:" + region_decode_ori(region_index_new, type, region_json) + "Now we aim to predict " \
